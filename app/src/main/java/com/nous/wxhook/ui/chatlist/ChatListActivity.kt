...
class ChatListActivity : Activity() {
    ...
    private fun loadConversations() {
        Thread {
            ...
            if (!File(dbPath).exists()) { handler.post { contentText.text = "数据库不存在，请先复制" }; return@Thread }

            val sqlFile = "/data/data/com.nous.wxhook/cache/q_${UUID.randomUUID()}.sql"
            try {
                File(sqlFile).writeText("PRAGMA key='$key';PRAGMA cipher_compatibility=3;PRAGMA cipher_page_size=1024;PRAGMA kdf_iter=4000;PRAGMA cipher_use_hmac=OFF;SELECT rconversation_username, rconversation_nickname, rconversation_unReadCount FROM rconversation ORDER BY rconversation_time DESC LIMIT 100;")
                ...
                if (lines.isEmpty()) sb.appendLine("无会话数据")
