...
class SettingsActivity : AppCompatActivity() {
    ...
    private fun showProviderInputDialog(name: String, provider: String) {
        if (provider != "s3" && provider != "webdav") return
        val ctx = this
        val col = android.widget.LinearLayout(ctx).apply {
            orientation = android.widget.LinearLayout.VERTICAL
            setPadding(40, 16, 40, 16)
        }
        // Collect input widgets: name -> view
        val inputs = mutableMapOf<String, View>()

        if (provider == "s3") {
            // ── 服务商 spinner ──
            val providers = listOf("AWS S3", "Cloudflare R2", "MinIO", "阿里云OSS", "腾讯COS", "华为OBS", "其他")
            val providerVals = listOf("AWS", "Cloudflare", "Minio", "Alibaba", "TencentCOS", "HuaweiOBS", "Other")
            col.addView(android.widget.TextView(ctx).apply { text = "服务商"; textSize = 13f })
            val pSpin = android.widget.Spinner(ctx)
            pSpin.adapter = android.widget.ArrayAdapter(ctx, android.R.layout.simple_spinner_dropdown_item, providers)
            col.addView(pSpin)
            inputs["s3_provider"] = pSpin

            // ── Access Key / Secret Key ──
            for ((k, label) in listOf("access_key_id" to "Access Key ID", "secret_access_key" to "Secret Access Key")) {
                col.addView(android.widget.TextView(ctx).apply { text = label; textSize = 13f })
                val et = android.widget.EditText(ctx).apply { textSize = 14f; setSingleLine(); setPadding(0, 4, 0, 8) }
                inputs[k] = et
                col.addView(et)
            }
            ...
            val regions = listOf("cn-north-1", "us-east-1", "ap-southeast-1", "eu-west-1", "auto", "其他")
            rSpin.adapter = android.widget.ArrayAdapter(ctx, android.R.layout.simple_spinner_dropdown_item, regions)
            rSpin.setSelection(0)
            col.addView(rSpin)
            inputs["region"] = rSpin

            // ── Endpoint ──
            col.addView(android.widget.TextView(ctx).apply { text = "Endpoint"; textSize = 13f })
            val ep = android.widget.EditText(ctx).apply { textSize = 14f; setSingleLine(); setPadding(0, 4, 0, 8) }
            inputs["endpoint"] = ep
            col.addView(ep)
            ...
            android.app.AlertDialog.Builder(ctx)
                ...
                .setPositiveButton("保存") { _, _ ->
                    ...
                    val s3prov = providerVals[pi]
                    val region = (inputs["region"] as android.widget.Spinner).selectedItem.toString()
                    var endpoint = (inputs["endpoint"] as android.widget.EditText).text.toString().trim()
                    val ak = (inputs["access_key_id"] as android.widget.EditText).text.toString().trim()
                    val sk = (inputs["secret_access_key"] as android.widget.EditText).text.toString().trim()

                    // Auto-fill endpoint if empty
                    if (endpoint.isEmpty()) {
                        endpoint = when (s3prov) {
                    ...
                    if (ak.isEmpty() || sk.isEmpty()) return@setPositiveButton
                    val sb = StringBuilder()
                    sb.appendLine("[$name]")
                    sb.appendLine("type = s3")
                    sb.appendLine("provider = $s3prov")
                    sb.appendLine("access_key_id = $ak")
                    sb.appendLine("secret_access_key = $sk")
                    ...
                    Thread {
                        try {
                            ...
                            runOnUiThread {
                                supportActionBar?.title = "设置 ✅ S3 配置已保存"
                                buildItems()
                            }
                        } catch (e: Exception) {
            ...
            val webdavFields = listOf(
                "url" to "WebDAV 地址",
                "user" to "用户名",
                "pass" to "密码"
            )
            val editTexts = mutableListOf<android.widget.EditText>()
            for ((key, label) in webdavFields) {
                col.addView(android.widget.TextView(ctx).apply { text = label; textSize = 13f })
                val et = android.widget.EditText(ctx).apply { textSize = 14f; setSingleLine(); setPadding(0, 4, 0, 8) }
                editTexts.add(et)
                col.addView(et)
            ...
            android.app.AlertDialog.Builder(ctx)
                ...
                .setView(col)
                .setPositiveButton("保存") { _, _ ->
                    var url = editTexts[0].text.toString().trim()
                    val user = editTexts[1].text.toString().trim()
                    val pass = editTexts[2].text.toString().trim()
                    if (url.isEmpty() || user.isEmpty()) return@setPositiveButton
                    if (!url.startsWith("http")) url = "https://$url"
                    val obscured = try {
                        val p = Runtime.getRuntime().exec(arrayOf(
                            BackupHookLocal.binPath + "/rclone", "obscure", pass
                        ))
                        p.inputStream.bufferedReader().readText().trim()
                    } catch (_: Exception) { pass }
                    val sb = StringBuilder()
                    sb.appendLine("[$name]")
                    sb.appendLine("type = webdav")
                    sb.appendLine("url = $url")
                    sb.appendLine("vendor = other")
                    ...
                    Thread {
                        try {
                            ...
                            rcloneCfgFile.writeText(existing + sb.toString())
                            runOnUiThread {
                                supportActionBar?.title = "设置 ✅ WebDAV 配置已保存"
                                buildItems()
                            }
                        } catch (e: Exception) {
                            runOnUiThread { supportActionBar?.title = "设置 ❌ 保存失败: ${e.message}" }
